...
For cross-project, cross-app, cross-machine, periodic, or operational work, read in order:

1. `~/masterindex/current/index.json`
2. `~/masterindex/tasks/index.json`
3. `~/masterindex/current/handoffs/index.json`

Use `entities[].id` as target keys. Apply active handoff directives addressed to the target entity or `all-entities`. Every entity is verified every six hours and refreshed daily. Update `current/index.json` before `current/inventory.md`; record gaps and ambiguities explicitly.
